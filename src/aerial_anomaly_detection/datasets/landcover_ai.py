"""
Module to implement a PyTorch dataset of LandCover.ai
"""
...
@dataclass
class LandCoverAI(torch.utils.data.Dataset):
    """
    A class to provide access to the LandCover.ai dataset.
    """
    ...
    def __post_init__(self) -> None:
        """
        Post-init method of the LandCover.ai dataset.
        """
        self.index_df = self.index_df[self.index_df['partition'] == self.partition]


    @classmethod
    def load(cls, root_folder : str | Path, partition : str) -> 'LandCoverAI':
        """
        Method to load the LandCover.ai dataset from a given path.
