"""
Module to implement a model trainer related to the MASATI (v2) dataset.
"""
...
class MASATIv2ModelTrainer(ModelTrainer):
    """
    A class to implement a model trainer related to the MASATI (v2) dataset.
    """
    ...
    def _test_step(self, reconstruction_error_threshold : float, epoch : int) -> Dict[str, Any]:
        ...
        Args:
            ...
            epoch (int): the epoch being run (tests are only run every 5 epochs).
        ...
        if epoch % 20 == 0:
            ...
            with torch.inference_mode():
                for _, X, y in tqdm(self.test_dataloader,
                    ...
                    ground_truth_anomaly = int(np.count_nonzero(y) > 0)
